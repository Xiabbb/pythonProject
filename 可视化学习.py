...
if __name__ == '__main__':
    ...
    st.radio('数据类型', ['排查', '溯源'], horizontal=True, index=1)
    st.radio('查询区域', ['浦东新区', '临港', '金山', '青浦', '松江', '奉贤', '嘉定工业区', '嘉定跨界河'],
             horizontal=True, index=5)
    # a=st.time_input('请输入账号id','Life of Brian')
    username = st.text_input('用户账号', 'xiabinbin')
    password = st.text_input('用户密码', '54188', type='password')
    t1 = st.date_input()
