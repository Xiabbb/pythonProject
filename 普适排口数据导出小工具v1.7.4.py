...
@Gooey(
    ...
    program_name="普适排口-数据导出工具 v1.7.4",  # 程序名称
...
def main():
    ...
    my_cool_parser.add_argument('time_2', help='请选择查询结束日期', widget="DateChooser",
                                default=str(datetime.date.today()))
    my_cool_parser.add_argument('username', help='请输入用户名', widget="PasswordField", default='psadmin')
    my_cool_parser.add_argument('password', help='请输入密码', widget="PasswordField", default='6Kjl7#Vf')
    # 获取参数
...
def suyuan_data(i, o, time_1, time_2):
    ...
    if keyword == "崇明区":
        info = session.post(url3_cm, json=data_info).json()['data']
    else:
    ...
    foundTime = '新增排口' if time_1 < info['foundTime'] < time_2 else ''
    ...
    try:
        unitName = suyuan['traceUnit'][0]['unitName']  # 责任主体
    except IndexError:
        unitName = ''
    b = ['350104', '350105']
    if keyword == "崇明区":
        logs = session.post(url4_cm, json=data_info).json()['data']
    else:
...
def paicha_data(i, o):
    ...
    suyuan = info['trace']
    userName = info['userName']
    foundTime = info['foundTime']
    riverName = paicha['riverName']
    riverCode = paicha['riverCode']
    regionName = paicha['regionName']
    lon = paicha['lon']
    lat = paicha['lat']
    investigateReportStatus = info['investigateReportStatus']
...
if __name__ == '__main__':
    ...
    url_cm_1 = 'http://139.224.72.67:8081/chongmingpkapi/app/login'
    url_cm_2 = 'http://139.224.72.67:8081/chongmingpkapi/outlet/approvalExport'
    url3_cm = 'http://139.224.72.67:8081/chongmingpkapi/outlet/outletDetailInfo'
    url4_cm = 'http://139.224.72.67:8081/chongmingpkapi/outlet/outletLogInfo'
    ...
    print(data_1)
    if keyword == '崇明区':
        token = session.post(url=url_cm_1, json=data_1).json()['data']['password']
    else:
    ...
    if keyword == '崇明区':
        a = session.post(url_cm_2, json=data)
    else:
